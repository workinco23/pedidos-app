              ...
              <div className="flex shrink-0 gap-2">
                ...
                {p.estado === "facturado" && (
                  <button
                    ...
                    disabled={cambiando.has(p.id)}
                    className="flex items-center gap-1 rounded-md px-3 py-1.5 text-xs font-bold text-white hover:brightness-95 disabled:opacity-60"
                    style={{ backgroundColor: "#059669" }}
                  >
                    <IconoCheck /> {cambiando.has(p.id) ? "Guardando..." : "Despachado"}
                  </button>
                )}
              </div>
            </div>
